  ...
  const KanbanCard = ({ o }) => {
    ...
    return (
      <div onClick={() => navigate(`/orders/${o.id}`)}
        style={{ background:'var(--cui-card-bg)', border:'1px solid var(--cui-border-color)', borderLeft:`3px solid var(--cui-${TYPE_COLOR[o.order_type] || 'secondary'})`, borderRadius:8, padding:'10px 12px', cursor:'pointer', marginBottom:8, transition:'box-shadow 0.15s' }}
    ...
    <>
        ...
        <CCardBody>
          <div className="d-flex gap-2 flex-wrap mb-3 align-items-center">
            <CInputGroup size="sm" style={{ width:240 }}>
              ...
              <CFormInput placeholder={t('orders.search')} value={search} onChange={e => setSearch(e.target.value)} />
            </CInputGroup>
      ...
      <CModal size="lg" visible={modal} onClose={handleModalClose}>
        ...
        <CForm onSubmit={handleCreate}>
          <CModalBody>
            <CRow className="g-3">
              <CCol xs={12}>
                ...
                <div className="d-flex flex-wrap gap-2">
                  {ORDER_TYPES.filter(tp => tp.key).map(tp => (
                    <CButton key={tp.key} size="sm" type="button" color={tp.color}
              ...
              <CCol xs={12} md={6}>
                ...
                <CFormInput value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} placeholder={t('orders.address_placeholder')} />
              </CCol>
              <CCol xs={6} md={3}>
          ...
          <CModalFooter>
            <CButton color="secondary" variant="outline" onClick={handleModalClose}>{t('common.cancel')}</CButton>
